...
this.updateVehicle = function(data) {
	...
	var vehicleNitro = document.querySelector('#nitro');
	/*var vehicleLocation = document.querySelector('#location');*/
	...
	if (data.status == true) {	
		...
		saferInnerHTML(document.querySelector('#location span'), data.location);*/
		...
		vehicleNitro.querySelector('span').style.height = data.nitro+'%';

		if (data.nitro <= 35) {
			if (vehicleNitro.classList.contains('dying') == false) { vehicleNitro.classList.add('dying');	}
		}
		else { vehicleNitro.classList.remove('dying'); }
