                ...
                if not seatbeltIsOn then
                    ...
                    if(beltWarningSet == false) then
                        if(currSpeed > 1 or currSpeed < -1) then
                            beltWarningSet = true
                            TriggerEvent("mole_notifications:SendNotification", {text = "Nezapomen se pripoutat!", type = "info", timeout = 1400})
			...
			if IsControlJustReleased(0,  Keys['B']) then
				...
				if(PedVehIsCar)then
					...
					if seatbeltIsOn then
						...
						TriggerEvent("mole_notifications:SendNotification", {text = "Pripoutal jsi se!", type = "success", timeout = 1400})
						TriggerServerEvent('esx_mole_misiones:PlayOnSource','buckle', 0.9)
					else
						TriggerEvent("mole_notifications:SendNotification", {text = "Odpoutal jsi se!", type = "error", timeout = 1400})
